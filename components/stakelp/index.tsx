...
const StakeLp: React.FC<StakeLpProps> = ({ toggle, setToggle, farm }) => {
    ...
    useEffect(() => {
        settokensAvailable(ethers.utils.formatUnits(farm.pool.stakedToken.balanceInUserWallet))
    }, [farm])
        ...
        <div tw=" flex flex-col px-2 py-4 bg-white space-y-6 opacity-100 border-4 rounded-xl background-color[#004FCE] border-white" >
            ...
            <div>
                ...
                <div tw="text-white text-right mt-2">
                    {active ? 
                    <span id="lpinfo">{parseFloat(tokensAvailable).toFixed(2)} {farm.basicInfo.first}-{farm.basicInfo.second} LP available</span> :
            ...
            <div tw="flex items-center justify-center cursor-pointer text-white fill-current hover:text-black">
                ...
                <span tw=" text-center text-xl mr-2">Get {farm.basicInfo.first}-{farm.basicInfo.second} LP</span>
