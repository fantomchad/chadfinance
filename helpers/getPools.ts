...
const getPools = async (userWalletAddress: string): Promise<number> => {
...
async function getTotalAllocPoints(masterContract): Promise<number> {
  ...
  return totalAllocPointsBigNumber.toNumber()
}
...
async function getRewardsPerBlock(masterContract): Promise<number> {
  ...
  return rewardsPerBlockBigNumber.toNumber()
}

async function getPoolsFromMasterChad(poolCount, userWalletAddress, masterContract, provider) {
  const poolPromises = []
  console.log("GONNE GET POOL", poolCount, "INFOS HERE:")
  ...
  console.log("The POOL INFOS HERE:", poolInfos)
}
...
async function getPoolInfo(poolIndex, userWalletAddress, masterContract, provider): Promise<Pool> {
  ...
  const pool: Pool = {
    ...
    tvl: 0,
    apr: 0,
    allocationPoints: poolInfo.allocPoint,
...
async function getToken(address, userWalletAddress, provider): Promise<Token | LpToken> {
  try {
    console.log("Checking if token is UNI token", address)
  ...
  try {
    console.log("Wasnt uni token, checking if it's ERC20")
